...
class Strategy:
    ...
    def check_exit_conditions(self, position):
        ...
        if pnl <= -1 * self.sl_pct * cost:
            return "SL"
        if pnl >= self.tp_pct * cost:
            return "TP"
...
class DirectionalStrategy(Strategy):
    ...
    def check_entry_signal(self, data):
        ...
        # Bullish Crossover
        if prev_short_ma <= prev_long_ma and last_short_ma > last_long_ma and last_rsi > config.RSI_OVERBOUGHT:
            return {"signal": "BUY", "strategy": "Bull Put Spread"}

        # Bearish Crossover
        if prev_short_ma >= prev_long_ma and last_short_ma < last_long_ma and last_rsi < config.RSI_OVERSOLD:
